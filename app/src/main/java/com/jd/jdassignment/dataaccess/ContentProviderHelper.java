...
import com.jd.jdassignment.common.ApplicationInstance;
...
public class ContentProviderHelper extends ContentProvider {
    ...
    public static final int MEETINGS                                = 8;
    public static final int MEETING_MEMBERS                         = 9;
    ...
    static {
        ...
        uriMatcher.addURI(LSCPConstants.PROVIDER_NAME, LSCPConstants.USERS_TABLE_NAME, USERNAME);
//        uriMatcher.addURI(LSCPConstants.PROVIDER_NAME, LSCPConstants.USERS_TABLE_NAME + TAG_ID_ALL, USER_ID);
        uriMatcher.addURI(LSCPConstants.PROVIDER_NAME, LSCPConstants.PATH_RELATIONSHIP_JOIN, RELATIONSHIP_JOIN);
    }

    private DataBaseHelper mOpenHelper;

    public static Uri getContentUri(int type) {
        String URL = LSCPConstants.CONTENT + LSCPConstants.PROVIDER_NAME;
        switch (type) {
            case USERNAME:
                URL += "/"+ UserDO.USERNAME;
                break;
    ...
    public Uri insert(Uri uri, ContentValues values) {
        synchronized (ApplicationInstance.LOCK_APP_DB) {
            ...
            if (_id > 0) {
                returnURI = LSCPConstants.buildUserUri(_id);
            } else {
                throw new SQLException("Failed to insert row into: " + uri);
            }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        synchronized (ApplicationInstance.LOCK_APP_DB) {
            ...
            switch (uriMatcher.match(uri)) {
                case USERNAME:
                    table = LSCPConstants.USERS_TABLE_NAME;
                    break;
                case USER_ID:
                    table = LSCPConstants.USERS_TABLE_NAME;
                    break;
    ...
    private String getTableName(Uri uri){
        String table = LSCPConstants.USERS_TABLE_NAME;
    ...
    public String getType(Uri uri) {
        ...
        switch (match) {
            case USERNAME:
                return LSCPConstants.CONTENT_MULTIPLE_TYPE;
            case USER_ID:
            case GROUP:
            case GROUP_MEMBER:
            case SYNCLOG:
            case IMAGES:
            case MEMBER:
                return LSCPConstants.CONTENT_BASE_TYPE;
            default: {
                throw new UnsupportedOperationException("Unknown uri: " + uri);
            }
        }
